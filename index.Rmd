...
Looking at the plots (slide 1) and histograms it is noticed that the playlists are distributed differently. Thus the folling hypothis were made regarding the changes in the discover weekly playlist over 8 weeks time (further explanation can be found below the tables):
...
In this project we will continuously compare these two songs, as they represent Alex's playlist very well, in the sense that a lot of different genres are compiled in his playlist. As they are completely different genres, we will see how different kind of feature analyses will work for the two songs.
...
If we look at the Timbre SSM  we see the breakdown at 60 seconds, this time even more clearly. As the song starts with some bass, high heads and some low vocals all of a sudden at 60 seconds a lot of low bass is added and this is easy to see by the yellow line at 60 seconds. Also we see a clear change at 244 seconds (4:02-4:05). This is because of that one weird vocal sample that is only played here.
...
Using Cosine as the distance for the timbre SSM, more structure is obtained, however some very big changes become less obvious, therefore an interactive timbre plot using Euclidean distance is shown here.
...
In the Additional Analyses, the results of the various analyses can be found. Showing the confusion matrixes and the accuracy tables. From these analysis it was found out that every algorithm is the best at correctly predicting songs that belong to my favorites. Why this is the case, I'm not sure.
...
### **ADDITIONAL RESULTS:** The following appendixes show various analyses of playlists en Lacrimosa VS Brainstorm (If you have time, take a look) **I: The Key of our Playlists** 
