...
discover_weekly7=
  get_playlist_audio_features("Tosca Beijaert","67TgsEi6QAM7fb7RB73N2q") %>%
  slice(1:30) %>%
  add_audio_analysis


Playlists <-
    favorites %>% mutate(playlist = "Favorites") %>%
    bind_rows(
        alex %>% mutate(playlist = "alex"),
        discover_weekly7 %>% mutate(playlist = "discover_weekly7")) %>%
...
Playlists_cv %>%
    mutate(pred = map(splits, Playlists_predict_knn)) %>% unnest(pred) %>%
    conf_mat(truth = playlist, estimate = .pred_class) %>%
    autoplot(type = 'heatmap') # Gives you the same confusion matrix as mosaic, but then a different plot that is more number directed.
...
The first table shows the confusion matrix, where we can see the amount of wrong and correct predictions for my favourites, alex's favoroutes and the discover weekly week 7 playlist. We can see that the accuracy is not that good. This can also be seen from the plots.

###  Multinomial Regression Prediction quality
...
First we see the amounts that were correctly and wrongly predicted. Also we see the accuracy. This shows us that multinomial regresion does a way better job than k-nearest neighbour.


### Multinomial Regression Shows us the most important contributing factors!
...
***
First we see the plots of the confusion matrix again. Under these two plots we have the multinomial regression which shows us which features contribute the most. Where very high or very low values contribute the most to making the correct predictions.

* We can see that for Alex's favorites: Speechiness, D#/Eb, c11 and c03are important for prediction.
* We can see that for Discover weekly 7: Danceability, Loudness, Instrumentalness and Duration were important for prediction.
* We can see that for My favorites: Danceability and c01, E, G#|Ab are important for prediction.
...
We can see that decision trees don't work that well. However we can use them to get a better view at how the decision for the predictions were made. For instance, we can see that danceability plays a very big role for this prediction. For instance, we see that when the component c04 <= -0.4251201, you belong to the playlist Favorites.


### Random Forests and important features
...
We can see that our Random forests do quite well. They have a high accuracy.
The last plot shows us the most important factors that we should look at. This shows us that danceability, loudness, duration, c04, c01 etc are very important. Now we can try using k-nearest neighbour again, but when we make our prediction model we won't use all the possible features, but pick only the most important ones,


### Feature Selection & Retrying the K-nearest neighbour
...
predict_knn_reduced <- function(split)
    fit(
        Playlists_knn,
        playlist ~ c01 + danceability + c04 + loudness + duration + instrumentalness, # Here he used less features, and gets better results.
        data = analysis(split)) %>%
    predict(assessment(split), type = 'class') %>%
    bind_cols(assessment(split))
...
Playlists %>%
    ggplot(aes(x = c01, y = c04, colour = playlist, size = danceability)) +
...
We see that the accuracy of our k-nearest neighbour has gone up quite a bit after selecting the important features.


### Novelty function, Chromagrams and Delta-delta's
...
Here we see the distributions of the keys per playlist at the baseline. You can see that for every playlist, a lot of the songs are in the key of C. However, there are also a lot for C#|Db, G & A.

### Alex's least Danceable Song: Lacrimosa
...
### Lacrimosa seems hard to analyze?
...
Now we can use Dynamic Time Warping to compute the most optimal alignment between the two features sequences. As chroma norm and distance measure manhattan was chosen, the other options did not seem to work well for this data. We can see the black line diagonally, expressing the optimal warping path.
...
The chroma plot did not give us much information, however the Chroma SSM gives us loads of info.
...
```{r chunk12, fig.height=10, fig.width=7}
library(lubridate)
...
features_longer_3$week <- factor(features_longer_3$week, levels = c("6","7","8","9","10","11", "12"), labels = c("Week 1","Week 2","Week 3","Week 4", "Week 5", "Week 6", "Week 7"))


# For the median
median_longer$week=as.character(median_longer$week)
median_longer$value=round(median_longer$value,2)
median_longer$week <- factor(median_longer$week, levels = c("6","7","8","9","10","11", "12"), labels = c("Week 1","Week 2","Week 3","Week 4", "Week 5", "Week 6", "Week7"))
require(dplyr)
library(CGPfunctions)
...
These Plot shows us the changes over the 5 weeks for each feature. The values shown are the means of the absolute values. The tempo values are devided by 100, else they wouldn't fit on the plot. Thus tempo= 1.16 equals tempo=116 (am still figuring out how to solve this).

On the left we have the mean values and on the right the median values. The first thing to notice when looking at the left Plot of the mean values is the big increase in instrumentalness for week one, which than starts to drop continuously untill it's lower in week 5, than the value we started with. Whereas the other variables are still fluctuating a lot over time, we can see that in general energy, danceability and tempo have gone up slightly.

However, these values only show us the mean values and we have seen before that the distribution of the data might be skewed, therefore we should look at other measures, like the median, which can be seen on the right. This plot shows us similar results..
...
ggplot(means_all, aes(week, danceability, col=instrumentalness, size=energy))+
  ...
  labs(title='Mean Changes per Week',
    ...
    size="Energy")  +
    scale_x_continuous(          # Fine-tune the x axis.
    breaks = c(6,7,8,9,10,11,12),  # Use grid-lines for quadrants only.
    minor_breaks = NULL,      # Remove 'minor' grid-lines.
    labels=week_numbers
  )
...
ggplot(discover_weekly7, aes(energy, danceability, col=instrumentalness, size=tempo))+
  geom_point()+
  labs(title='Discover Weekly 7',
    x = "Energy",
    y = "Danceability",
    colour = "Instrumentalness",
    size="Tempo")  +
...
These plot respresent week 1 and week 7 of discover weekly, showing the distribution of all 4 features.
