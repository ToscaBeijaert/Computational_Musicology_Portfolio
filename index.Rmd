...
playlists_end=
  ...
  bind_rows(discover_weekly5%>% mutate(playlist = "Discover Weekly 5"))
...
Here we see the distributions of the keys per playlist. You can see that for every playlist, a lot of the songs are in the key of C. However, there are also a lot for C#|Db, G & A.
...
features_longer_3$week <- factor(features_longer_3$week, levels = c("6","7","8","9","10","11"), labels = c("Week 1","Week 2","Week 3","Week 4", "Week 5", "Week 6"))


# For the median
median_longer$week=as.character(median_longer$week)
median_longer$value=round(median_longer$value,2)
median_longer$week <- factor(median_longer$week, levels = c("6","7","8","9","10","11"), labels = c("Week 1","Week 2","Week 3","Week 4", "Week 5", "Week 6"))
require(dplyr)
library(CGPfunctions)
...
ggplot(means_all, aes(week, danceability, col=instrumentalness, size=energy))+
  ...
  labs(title='Mean Changes per Week',
    ...
    size="Energy")  +
    scale_x_continuous(          # Fine-tune the x axis.
    breaks = c(6,7,8,9,10,11),  # Use grid-lines for quadrants only.
    minor_breaks = NULL,      # Remove 'minor' grid-lines.
    labels=week_numbers
  )
...
ggplot(discover_weekly6, aes(energy, danceability, col=instrumentalness, size=tempo))+
  geom_point()+
  labs(title='Discover Weekly 6',
    x = "Energy",
    y = "Danceability",
    colour = "Instrumentalness",
    size="Tempo")  +
...
These plot respresent week 1 and week 6 of discover weekly, showing the distribution of all 4 features.
