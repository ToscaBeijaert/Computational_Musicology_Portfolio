...
### **Will my Discover Weekly transform into Alex? Differences for Danceability, Energy, Tempo and Instrumentalness**{data-commentary-width=305}
...
<!-- ### **The Baseline differences: the distributions of the features**{data-commentary-width=300} -->
...
<!-- ### **Alex's least Danceable Song VS Alex's most danceable Song: Larimosa VS Brainstorm!** -->
...
<!-- Now I know that Lacrimosa is played in d-minor, which will help us later on. THe songs has some very particular Harmonic orders, for instance for the first few beats: I-II-V-I-II-V. This can also be seen in the picture. -->
...
### **Dynamic Time Warping the two Lacrimosa's. That seems to work!**{data-commentary-width=400}
...
### **SSM matrixes Showing us what's actually going on.**{data-commentary-width=310}
...
Lacrimosa_Chroma %>%
    ...
    scale_fill_viridis_c(option = 'B', guide = 'none') +
      theme_classic()+
  labs(title='Lacrimosa Chroma SSM', x="", y="")
...
SSM_Chroma=Brainstorm_Chroma %>%
  ...
  labs(title='Brainstorm Chroma SSM', x="", y="")
...
Lacrimosa_Timbre %>%  # Timbre self similairity matrix
    ...
    scale_fill_viridis_c(option = 'B', guide = 'none') +
    theme_classic() +
    labs(title="Lacrimosa Timbre SSM Using Cosine Distance",x = 'Time (s)', y = 'Time (s)')
...
Brainstorm_Timbre %>%  # Timbre self similairity matrix
    ...
    scale_fill_viridis_c(option = 'B', guide = 'none') +
    theme_classic() +
    labs(title="Brainstorm Timbre SSM Using Cosine Distance",x = 'Time (s)', y = 'Time (s)')+
  scale_x_continuous(
...
### **Novelty functions and Tempograms for Lacrimosa & Brainstorm. Can the beats be derived?**{data-commentary-width=310}
...
__Novelty Functions__
For Lacrimosa the novelty function is not very consistent, the first novelty plot show the first 30s, which start with 10s of only violin and then the vocals come in.
...
<!-- ### **Tempograms: Lacrimosa stays unclear!**{data-commentary-width=298} -->
...
### **Classification with  Multinomial Regression. What features are important for what playlist?**{data-commentary-width=345}
...
The accuracy showedt hat k-nearest neighbour doesn't do that well. All the others perform about the same, with Random forest doing slightly better. Also we saw that k-nearest neighbour improved after only the important features according to the random forest were used being: Danceability + c04 + duration + tempo + loudness
...
In Appendix D, the results of the various analyses can be found. Showing the confusion matrixes and the accuracy tables. From these analysis it was found out that every algorithm is the best at correctly predicting songs that belong to my favorites. Why this is the case, I'm not sure.



### **Using Multinomial selected features shows us Clusters!** {data-commentary-width=711}
...
### Appendix A: **The Key of our Playlists**
...
### Appendix B: **Chroma, Timbre & Key plots for Lacrimosa & Brainstorm**{data-commentary-width=290}
...
### Appendix C: **Looking at important Features using Multinomial Regression, Decision Trees & Random Forests**{data-commentary-width=300}
...
### Appendix D: **Classification with k-Nearest Neighbour, Multinomial Regression, Decision Trees and Random forest**{data-commentary-width=700}
...
The accuracy of different predictions methods were looked at, finding out the following accuracies:
...
On the left we see a a plot showing the confusion matrix for the Multinomial Regression, as we will later use the most meaningfull feaatures according to the multinomial regression to look at clusters in the Discover Weekly 8 playlist. This plot clearly show that it's best at correctly classifying songs from my favorites.
...
### Appendix E: **What about techno/breakbeat songs? Introducing Brainstorm from Special Request! The Chroma does not say much, Timbre to the rescue!**
...
### Appendix F: **Novelty functions, Chromagrams and Delta-delta's for Lacrimosa & Brainstorm. Can the beats be derived?**
