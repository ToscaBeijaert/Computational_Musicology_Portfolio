...
### **Will my Discover Weekly transform into Alex? Differences for Danceability, Energy, Tempo and Instrumentalness**{data-commentary-width=280}
...
### **The Chroma of Lacrimosa seems hard to analyze?**{data-commentary-width=290}
...
### **SSM matrixes Showing us what's actually going on.**{data-commentary-width=295}
...
On the left we see the accuracy for 1: K-nearest neighbour, 2: Multinomial regression, 3:Decision Trees, 4:Random Forest and 5:K-nearest neighbour with important features. The accuracy shows us that k-nearest neighbour doesn't do that well. All the others perform about the same. However we do see that k-nearest neighbour improves after only the important features from the multinomial regression are used. In general, it's easiest to correctly predict which songs belong to my favorites.

Underneath those tables there is a plot showing the confusion matrix for the Multinomial Regression, as we will later use the most meaningfull feaatures according to the multinomial regression to look at clusters in the Discover Weekly 8 playlist. This plot clearly show that it's best at correctly classifying songs from my favorites.
...
### **Using GLM selected features shows us Clusters!** {data-commentary-width=700}
